   ...
   for i=1:N-1
        if i==1
         ...
         self=-2*(betac*Ax*permx(1,i+1))/(mu(pressure(n-1,i+1))*Bo(pressure(n-1,i+1))*(deltax(1,i+1)))-(vb(1,i+1)*phi*co)/(alphac*Bo(pressure(n-1,i+1))*deltat); 
           zarib(i,i+1)=right;
           zarib(i,i)=self;
           sabet(i,1)=-qsc(i+1)-(vb(1,i+1)*phi*co)/(alphac*Bo(pressure(n-1,i+1))*deltat)*pressure(n-1,i+1)-left*pl;
           

      elseif i==N-1
         right=(betac*Ax*(deltax(1,i+1)+deltax(1,i+1))/(deltax(1,i+1)/permx(1,i+1)+deltax(1,i+1)/permx(1,i+1)))/(mu(1/2*(pressure(n-1,i+1)+pressure(n-1,i+1)))*Bo(1/2*(pressure(n-1,i+1)+pressure(n-1,i+1)))*(1/2*(deltax(1,i+1)+deltax(1,i+1))));
         left=(betac*Ax*(deltax(1,i+1)+deltax(1,i))/(deltax(1,i+1)/permx(1,i+1)+deltax(1,i)/permx(1,i)))/(mu(1/2*(pressure(n-1,i+1)+pressure(n-1,i)))*Bo(1/2*(pressure(n-1,i+1)+pressure(n-1,i)))*(1/2*(deltax(1,i+1)+deltax(1,i))));
         self=-2*(betac*Ax*permx(1,i+1))/(mu(pressure(n-1,i+1))*Bo(pressure(n-1,i+1))*(deltax(1,i+1)))-(vb(1,i+1)*phi*co)/(alphac*Bo(pressure(n-1,i+1))*deltat); 
           zarib(i,i-1)=left;
           zarib(i,i)=self+right;
           sabet(i,1)=-qsc(1,i+1)-(vb(1,i+1)*phi*co)/(alphac*Bo(pressure(n-1,i+1))*deltat)*pressure(n-1,i+1);
        else
