...
for n=2:nt
    ...
    for i=1:N-1 
        ...
        elseif i==N-1
%         right=(betac*Ax*(deltax(1,i)+deltax(1,i+1))/(deltax(1,i)/permx(1,i)+deltax(1,i+1)/permx(1,i+1)))/(mu*Bo(1/2*(pressure(n-1,i)+pressure(n-1,i+1)))*(1/2*(deltax(1,i)+deltax(1,i+1))));
         left=(betac*Ax*(deltax(1,i+1)+deltax(1,i))/(deltax(1,i+1)/permx(1,i+1)+deltax(1,i)/permx(1,i)))/(mu(1/2*(pressure(n-1,i+1)+pressure(n-1,i)))*Bo(1/2*(pressure(n-1,i+1)+pressure(n-1,i)))*(1/2*(deltax(1,i+1)+deltax(1,i))));

        pressure(n,i+1)=pressure(n-1,i+1)+(alphac*Bo(pressure(n-1,i+1))*deltat)/(vb(1,i+1)*phi*co)*q(1,i+1)+(alphac*Bo(pressure(n-1,i+1))*deltat)/(vb(1,i+1)*phi*co)*((-(left)*(pressure(n-1,i+1)))+(left*pressure(n-1,i)));   
